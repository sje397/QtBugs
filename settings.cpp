...
Settings::Settings(QWidget *parent): QWidget(parent) {
	...
	ui.minBugEnergyEd->setValidator(new QDoubleValidator(this));
	ui.maxBugEnergyEd->setValidator(new QDoubleValidator(this));
	ui.childEnergyEd->setValidator(new QDoubleValidator(this));
	ui.moveEnergyEd->setValidator(new QDoubleValidator(this));
	ui.stayEnergyEd->setValidator(new QDoubleValidator(this));
	ui.minNodeEnergyEd->setValidator(new QDoubleValidator(this));
	ui.maxNodeEnergyEd->setValidator(new QDoubleValidator(this));
	ui.totalEnergyEd->setValidator(new QDoubleValidator(this));
	ui.minTotalBugEd->setValidator(new QDoubleValidator(this));
	ui.minTotalNodeEd->setValidator(new QDoubleValidator(this));
...
void Settings::apply() {
	...
	if(ok) newParams.min_energy = ui.minBugEnergyEd->text().toFloat(&ok);
	if(ok) newParams.max_energy = ui.maxBugEnergyEd->text().toFloat(&ok);
	if(ok) newParams.child_energy = ui.childEnergyEd->text().toFloat(&ok);
	if(ok) newParams.move_energy = ui.moveEnergyEd->text().toFloat(&ok);
	if(ok) newParams.stay_energy = ui.stayEnergyEd->text().toFloat(&ok);
	if(ok) newParams.min_e_energy = ui.minNodeEnergyEd->text().toFloat(&ok);
	if(ok) newParams.max_e_energy = ui.maxNodeEnergyEd->text().toFloat(&ok);
	if(ok) newParams.total_energy = ui.totalEnergyEd->text().toFloat(&ok);
	if(ok) newParams.min_bug_energy = ui.minTotalBugEd->text().toFloat(&ok);
	if(ok) newParams.min_energy_energy = ui.minTotalNodeEd->text().toFloat(&ok);
